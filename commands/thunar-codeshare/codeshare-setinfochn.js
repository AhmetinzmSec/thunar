...
exports.run = (client, message, args) => {
    ...
    if (args[0] === "ayarla" || args[0] == "set") {
        ...
        db.set("cscn" + message.guild.id, channel.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik Kazanıldı')
            .addField(`Sunucunun log kanalı ${channel.name} oldu`, 'Artık yeni kod kaydı bu kanala mesajlı bildirim şeklinde tutulacak')
            .setColor(renk)
            .setFooter(slogan)
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("cscn" + message.guild.id)) return message.channel.send("Sistem zaten kapalı")

        db.delete("cscn" + message.guild.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik İptal Edildi')
            .addField(`Sunucunun log kanalı silindi`, 'Artık yeni kod kaydı tutulmayacak')
            .setColor(renk)
            .setFooter(slogan)
        message.channel.send(embed)
    }
...
exports.conf = {
    aliases: ['cs', 'code-share', 'codeshare'],
