...
exports.run = (client, message, args) => {
    ...
    const kanalyok = new discord.MessageEmbed()
        ...
        .setDescription('Bir çıktı üssü ayarı yapmam için işlem belirtilmedi. Ayarla/Sil işlemlerinden birini belirtin')
        .setColor(renk)
        .setFooter(slogan)
    ...
    if (args[0] === "ayarla" || args[0] == "set") {
        ...
        db.set("botinfo" + message.guild.id, channel.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik Kazanıldı')
            .addField(`Sunucunun botlist çıktı üssü ${channel.name} oldu`, 'Artık bot ekleme isteği bu kanala işlenecek')
            .setColor(renk)
            .setFooter(slogan)
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("botinfo" + message.guild.id)) return message.channel.send("Sistem zaten kapalı")

        db.delete("botinfo" + message.guild.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik İptal Edildi')
            .addField(`Sunucunun botlist çıktı üssü silindi`, 'Artık botlist kaydı tutulmayacak')
            .setColor(renk)
            .setFooter(slogan)
        message.channel.send(embed)
    }
...
exports.conf = {
    aliases: ['bot-list-bilgi', 'bl-bilgiüssü'],
