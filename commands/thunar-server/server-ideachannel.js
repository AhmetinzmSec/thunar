...
exports.run = (client, message, args) => {
    ...
    if (args[0] === "ayarla" || args[0] == "set") {
        ...
        db.set("ideachannel" + message.guild.id, channel.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik Kazanıldı')
            .addField(`Sunucunun log kanalı ${channel.name} oldu`, 'Artık önermeler bu kanaldan okunacak')
            .setColor(renk)
            .setFooter(slogan)
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("ideachannel" + message.guild.id)) return message.channel.send("Sistem Zaten Kapalı")

        db.delete("ideachannel" + message.guild.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik İptal Edildi')
            .addField(`Sunucunun log kanalı silindi`, 'Artık önermeler yazılamayacak')
            .setColor(renk)
            .setFooter(slogan)
        message.channel.send(embed)
